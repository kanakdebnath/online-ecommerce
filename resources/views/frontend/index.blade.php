            ...
            <section class="product-tabs section-padding position-relative">
                <div class="section-title style-2">
                    ...
                    <ul class="nav nav-tabs links" id="myTab" role="tablist">
                        ...
                        @php
                            $categories = App\Models\Admin\Category::where('status','active')->latest()->take(2)->get();
                        @endphp
                        @foreach ($categories as $item)
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="nav-tab-two" data-bs-toggle="tab" data-bs-target="#tab-two-{{$item->id}}" type="button" role="tab" aria-controls="tab-two-{{$item->id}}" aria-selected="false">{{ $item->name }}</button>
                            </li>
                        @endforeach
                ...
                @php
                    $products = App\Models\Admin\Product::where('status','active')->latest()->take(10)->get();
                @endphp
                ...
                <div class="tab-content" id="myTabContent">
                                    ...
                                    <div class="product-img-action-wrap">
                                        <div class="product-img product-img-zoom">
                                            <a href="{{ route('product-details',$product->id) }}">
                                                <img class="default-img" src="{{ asset('uploads/product').'/'.$product->photo }}" alt="{{ $product->name }}" />
                                                <img class="hover-img" src="{{ asset('uploads/product').'/'.$product->photo }}" alt="{{ $product->name }}" />
                                            </a>
                                        </div>
                                    ...
                                    <div class="product-content-wrap">
                                        ...
                                        <div class="product-card-bottom">
                                            <div class="product-price">
                                                <span>{{ get_discount_price($product->id) }}</span>
                                                <span class="old-price">{{ $product->price }}</span>
                                            </div>
                                            <div class="add-cart">
                                                <a class="add" href="{{ route('addToCart',$product->id) }}"><i class="fi-rs-shopping-cart mr-5"></i>Add </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @foreach ($categories as $item)
                        @php
                            $products = App\Models\Admin\Product::where('status','active')
                            ->where('category_id',$item->id)
                            ->latest()->take(10)->get();
                        @endphp
                    <div class="tab-pane fade" id="tab-two-{{ $item->id }}" role="tabpanel" aria-labelledby="tab-two">
                        <div class="row product-grid-4">
                            @foreach ($products as $product)
                            <div class="col-lg-1-5 col-md-4 col-12 col-sm-6">
                                <div class="product-cart-wrap mb-30">
